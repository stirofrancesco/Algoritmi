...
template <class H> class Coppia{
  ...
  public:
    ...
    bool operator >(Coppia& c){
      ...
      else if(second!=c.getSecond()) return second > c.getSecond();
      else return third > c.getThird();
    }
};
...
void heapSort(Coppia<H>** V, int n, ofstream& out){
  ...
  for(int i=0;i<n;i++)
    out << "(" << V[i]->getFirst() << "," << V[i]->getSecond() << "," << V[i]->getThird() << ")" << " ";
  out << endl;
}
